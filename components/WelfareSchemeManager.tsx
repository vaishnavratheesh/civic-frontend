...
import CreateWelfareScheme from './CreateWelfareScheme';
...
interface WelfareSchemeManagerProps {
  onSchemeCreated: () => void;
}

const WelfareSchemeManager: React.FC<WelfareSchemeManagerProps> = ({ onSchemeCreated }) => {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Welfare Schemes</h2>
          <p className="text-gray-600">
            {user?.role === 'admin' 
              ? 'Manage all welfare schemes across the panchayath' 
              : 'Manage your ward-specific schemes and view panchayath-wide schemes'
            }
          </p>
        </div>
        <button
          onClick={() => setShowCreateForm(true)}
          className="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-200 font-medium shadow-lg shadow-blue-500/25 flex items-center"
        >
        ...
        {filteredSchemes.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              {searchTerm || filterScope !== 'all' || filterCategory !== 'all' || filterStatus !== 'all'
                  ...
                  <tr key={scheme._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex space-x-2">
                        <button
                          onClick={() => updateSchemeStatus(scheme._id, scheme.status === 'active' ? 'inactive' : 'active')}
                        ...
                        {(user?.role === 'admin' || scheme.createdBy === 'councillor') && (
                          <button
...
export default WelfareSchemeManager; 